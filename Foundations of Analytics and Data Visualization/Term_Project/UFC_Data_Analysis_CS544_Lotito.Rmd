...
Now that we know that red fighters tend to win more often, both in title-fights and non-title fights I wanted to analyze what the winning streak looks like for both red and blue fighters. The analysis tells us that for both red and blue fighters the resultsa are about the same with a mean of around 1.3 and a max streak of 16 for both. The distributions are also identical.

```{r, echo = FALSE}

win_streak <- ufc_data[c("R_current_win_streak")]


barplot(table(win_streak), col = c("Red"), xlab = "Red # of Wins in Steak", ylab = "Frequency", main = "Duration of Win Streak: Red Fighters")
...
barplot(table(win_streak), col = c("Blue"), xlab = "Blue # of Wins in Steak", ylab = "Frequency", main = "Duration of Win Streak: Blue Fighters")

table(win_streak_blue)
summary(win_streak)
```
