...
# Prob of getting a letter grade conditional of prev letter grade
prop.table(y,1)
...
# Prob of getting a prev letter grade conditional of the current grade
prop.table(y,2)
...
barplot(table(central.park.cloud), col = "blue", xlim = c(0, 12), xlab = "Frequency", ylab = "Type", horiz = TRUE)
########################################################
...
barplot(MAX, names.arg = 1:31, xlab = "Day", ylab = "Max Temp", col = "green", las=2)

barplot(MIN, names.arg = 1:31, xlab = "Day", ylab = "Min Temp", col = "green", las=2)

barplot(AVG, names.arg = 1:31, xlab = "Day", ylab = "Avg Temp", col = "green", las=2)
####################
#### Histograms ####
####################
...
hist(Release.Year, col=hcl(0), ylim = c(0,34), breaks = 5)

barplot(Release.Year, names.arg = 1:79, xlab = "Year", ylab = "Number", col = "red", horiz = TRUE, ylim = c(0,12))

(x <- hist(Gross, plot = TRUE))
names(x)
...
Gross[which((Gross > 150) & (Gross < 200))]
...
margin.table(x)
# Prob of Parent buckling/unbuckled conditional of Child buckled/unbuckled
prop.table(x,1)

# Prob of Child buckling/unbuckled conditional of Parent buckled/unbuckled
prop.table(x,2)
...
barplot(table(central.park.cloud), col = "cyan", ylim = c(0, 12), xlab = "Type", ylab = "Frequency", main = "Test", horiz = T, las = 2)
...
pie(table(central.park.cloud), col=hcl(c(0,60,120)))
...
barplot(central.park$MIN, names.arg=1:31, xlab="Day", ylab="Min Temp", col="cyan", las=2)

dotplot(central.park$MIN, labels=1:31, xlab="Min Temp")


hist(alltime.movies$Gross, col=hcl(0), ylim = c(0,35))

x <- hist(alltime.movies$Gross, col=hcl(0), ylim = c(0,35))


names(x)
x$breaks

x2<- hist(alltime.movies$Gross, col=hcl(0), ylim = c(0,35), breaks = seq(150, 650, 25))

x2$counts


(min <- summary(central.park$MIN))

central.park$MIN[c(which(central.park$MIN > (min[4] + 1.5*(min[4] - min[2]))), which(central.park$MIN < (min[4] - 1.5*(min[4] - min[2]))))]


(x <- c(7, 2, 6, 2, 3, 8, 2, 2, 7, 5, 2, 6, 9, 5))
...
If a boxplot is created with the following data,

(x <- c(46, 28, 16, 58, 58, 34, 70, 24, 54, 98))
(x <- c(54, 90, 74, 74, 28, 80, 56, 14, 62, 32))

summary(x)
fivenum(x)

(x <- c(89, 44, 72, 57, 25, 96, 65, 70, 60, 71, 16, 49))

(x <- c(7, 3, 8, 8, 6, 6, 3, 9, 1, 2, 6, 2))

(x <- c(6,1,9,7,4,8,2,7,2,9,1,9,3,4,2))

(x <- c(7, 2, 6, 2, 3, 8, 2, 2, 7, 5, 2, 6, 9, 5))

barplot(x, ylim = c(1,9))
