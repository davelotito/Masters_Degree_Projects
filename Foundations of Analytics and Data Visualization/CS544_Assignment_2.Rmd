...
aapl <- read.csv("https://people.bu.edu/kalathur/datasets/AAPL.csv")
...
c(sprintf("The minimum Tesla value of %g is at row %d on %s", min(tsla$Close), as.numeric(rownames(tsla[which(tsla$Close == min(tsla$Close)), ])[1]), tsla[which(tsla$Close == min(tsla$Close)), 1][1]), sprintf("The minimum Tesla value of %g is at row %d on %s", min(tsla$Close), as.numeric(rownames(tsla[which(tsla$Close == min(tsla$Close)), ])[2]), tsla[which(tsla$Close == min(tsla$Close)), 1][2]))

## c) Produce the output for the maximum price of the Tesla closing value in the dataset as shown below:

sprintf("The minimum Tesla value of %g is at row %d on %s", max(tsla$Close), as.numeric(rownames(tsla[which(tsla$Close == max(tsla$Close)), ])[1]), tsla[which(tsla$Close == max(tsla$Close)), 1][1])
...
(volume_greater_than_20m <- volume_greater_than_20m[c('Volume')])

(prob_volume_greater_than_20m <- nrow(volume_greater_than_20m) / nrow(tsla))
...
(nrow((subset(tsla, Close > Open))) / nrow(tsla)) / (nrow(subset(tsla, Volume > 20000000)) / nrow(tsla))
...
((nrow(tsla) * tail(tsla[,c("Close")], n=1)) - sum(tsla$Low)) / sum(tsla$Low)
